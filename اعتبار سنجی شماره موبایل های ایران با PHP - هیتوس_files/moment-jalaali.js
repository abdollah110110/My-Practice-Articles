'use strict';(function(root,factory){if(typeof define==='function'&&define.amd){define(['moment'],function(moment){root.moment=factory(moment)
return root.moment})}else if(typeof exports==='object'){module.exports=factory(require('moment'))}else{root.moment=factory(root.moment)}})(this,function(moment){if(moment==null){throw new Error('Cannot find moment')}
var formattingTokens=/(\[[^\[]*\])|(\\)?j(Mo|MM?M?M?|Do|DDDo|DD?D?D?|w[o|w]?|YYYYY|YYYY|YY|gg(ggg?)?|)|(\\)?(Mo|MM?M?M?|Do|DDDo|DD?D?D?|ddd?d?|do?|w[o|w]?|W[o|W]?|YYYYY|YYYY|YY|gg(ggg?)?|GG(GGG?)?|e|E|a|A|hh?|HH?|mm?|ss?|SS?S?|X|zz?|ZZ?|.)/g,localFormattingTokens=/(\[[^\[]*\])|(\\)?(LT|LL?L?L?|l{1,4})/g,parseTokenOneOrTwoDigits=/\d\d?/,parseTokenOneToThreeDigits=/\d{1,3}/,parseTokenThreeDigits=/\d{3}/,parseTokenFourDigits=/\d{1,4}/,parseTokenSixDigits=/[+\-]?\d{1,6}/,parseTokenWord=/[0-9]*['a-z\u00A0-\u05FF\u0700-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF]+|[\u0600-\u06FF\/]+(\s*?[\u0600-\u06FF]+){1,2}/i,parseTokenTimezone=/Z|[\+\-]\d\d:?\d\d/i,parseTokenT=/T/i,parseTokenTimestampMs=/[\+\-]?\d+(\.\d{1,3})?/,unitAliases={jm:'jmonth',jy:'jyear'},formatFunctions={},ordinalizeTokens='DDD w M D'.split(' '),paddedTokens='M D w'.split(' '),formatTokenFunctions={jM:function(){return this.jMonth()+1},jMMM:function(format){return this.lang().jMonthsShort(this,format)},jMMMM:function(format){return this.lang().jMonths(this,format)},jD:function(){return this.jDate()},jDDD:function(){return this.jDayOfYear()},jw:function(){return this.jWeek()},jYY:function(){return leftZeroFill(this.jYear()%100,2)},jYYYY:function(){return leftZeroFill(this.jYear(),4)},jYYYYY:function(){return leftZeroFill(this.jYear(),5)},jgg:function(){return leftZeroFill(this.jWeekYear()%100,2)},jgggg:function(){return this.jWeekYear()},jggggg:function(){return leftZeroFill(this.jWeekYear(),5)}},i
...
function compareArrays(array1,array2){var len=Math.min(array1.length,array2.length),lengthDiff=Math.abs(array1.length-array2.length),diffs=0,i
for(i=0;i<len;i+=1)
if(~~array1[i]!==~~array2[i])
diffs+=1
return diffs+lengthDiff}
function normalizeUnits(units){return units?unitAliases[units]||units.toLowerCase().replace(/(.)s$/,'$1'):units}
function setDate(moment,year,month,date){var utc=moment._isUTC?'UTC':''
...
case'a':case'A':return moment.langData(config._l)._meridiemParse
...
break
case'jMMM':case'jMMMM':a=moment.langData(config._l).jMonthsParse(input)
if(a!=null)
datePartArray[1]=a
else
...
function makeDateFromStringAndArray(config,utc){var tempConfig,tempMoment,bestMoment,scoreToBeat=99,len=config._f.length,i,currentScore,format
...
if(currentScore<scoreToBeat){scoreToBeat=currentScore
...
function makeMoment(input,format,lang,utc){var config={_i:input,_f:format,_l:lang},date,m,jm
...
extend(jm,m)
return jm}
function jMoment(input,format,lang){return makeMoment(input,format,lang,false)}
extend(jMoment,moment)
jMoment.fn=objectCreate(moment.fn)
jMoment.utc=function(input,format,lang){return makeMoment(input,format,lang,true)}
jMoment.fn.format=function(format){var i,replace,me=this
if(format){i=5
replace=function(input){return me.lang().longDateFormat(input)||input}
...
jMoment.fn.jWeek=function(input){var week=jWeekOfYear(this,this.lang()._week.dow,this.lang()._week.doy).week
return input==null?week:this.add('d',(input-week)*7)}
jMoment.fn.jWeekYear=function(input){var year=jWeekOfYear(this,this.lang()._week.dow,this.lang()._week.doy).year
return input==null?year:this.add('y',input-year)}
jMoment.fn.add=function(val,units){var temp
if(typeof val==='string'){temp=val
val=units
units=temp}
units=normalizeUnits(units)
if(units==='jyear'){this.jYear(this.jYear()+val)}else if(units==='jmonth'){this.jMonth(this.jMonth()+val)}else{moment.fn.add.call(this,units,val)}
return this}
...
jMoment.jDaysInMonth=function(year,month){if(month<6){return 31}else if(month<11){return 30}else if(jMoment.jIsLeapYear(year)){return 30}else{return 29}}
jMoment.jIsLeapYear=function(year){return jalCal(year).leap===0}
...
function toJalaali(gy,gm,gd){var j=d2j(g2d(gy,gm+1,gd))
j.jm-=1
return j}
function toGregorian(jy,jm,jd){var g=d2g(j2d(jy,jm+1,jd))
g.gm-=1
return g}
